pipeline {
    agent any
    environment{
        IMAGE_NAME= 'pradippipaliya/react_app:1.2'
    }
            ...
            steps {
                script {
                    ...
                    sshagent(['ec2-linux-key']) {
                        sh "ssh -o StrictHostKeyChecking=no ec2-user@35.175.246.184  ${dockerStop}"
                        sh "ssh -o StrictHostKeyChecking=no ec2-user@35.175.246.184  ${dockerDelete}"
